...
class SerialReader:
    ...
    def _start_session(self, serial_dev, serial_fd_type='u', client_id=0):
        if serial_fd_type=='r':
            self.serialqueue = self.ffi_main.gc(
                self.ffi_lib.serialqueue_alloc_child(serial_dev),
            ...
            self.serialqueue = self.ffi_main.gc(
                ...
                self.ffi_lib.serialqueue_free)
            serialports[serial_dev.port] = self.serialqueue
    ...
    def connect_uart(self, serialport, baud, rts=True):
        ...
        while 1:
            if self.reactor.monotonic() > start_time + 90.:
                raise error("Unable to connect")
            if ":child" in serialport:
                parent, client_id = serialport.rsplit(':child', 1)
                if parent not in serialports:
                    raise error("Unknown serial port" % (parent,))
                serial_dev = serialports[parent]
                serial_fd_type = 'r'
                logging.info("Routing serial connection via " + parent)
            else:
                serial_fd_type = 'u'
                client_id = 0
                try:
            ...
            ret = self._start_session(serial_dev, serial_fd_type, client_id)
            if ret:
                break
