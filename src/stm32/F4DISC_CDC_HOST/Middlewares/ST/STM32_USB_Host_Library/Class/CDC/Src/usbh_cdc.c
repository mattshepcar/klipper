...
static uint8_t cdchandleused = 0;
static CDC_HandleTypeDef cdchandle;
...
static USBH_StatusTypeDef USBH_CDC_InterfaceInit(USBH_HandleTypeDef *phost)
{
  ...
  phost->pActiveClass->pData = cdchandleused ? NULL : &cdchandle;
  cdchandleused = 1;
...
* @retval None
*/
__weak void USBH_CDC_ReceiveCallback(USBH_HandleTypeDef *phost, int length)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(phost);
}
